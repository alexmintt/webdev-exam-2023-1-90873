...
function updatePricing(guide) {
  ...
  const excursionTime = document.getElementById('excursionTime').value.slice(0, 2);
  const option1 = document.getElementById('additionalOption1');
  const option2 = document.getElementById('additionalOption2');
  let dayMod = 1;
  if (new Date(excursionDate).getDay() === 6 || new Date(excursionDate).getDay() === 0) {
    dayMod = 1.5;
  }
  let cost = guide.pricePerHour * hours * dayMod;
  if (parseInt(excursionTime, 10) >= 9 && parseInt(excursionTime, 10) <= 12) {
    cost += 400;
  } else if (parseInt(excursionTime, 10) >= 20 && parseInt(excursionTime, 10) <= 23) {
    cost += 1000;
  }
  if (groupSize >= 10) {
    cost += 1500;
...
function openBookingModal(guide, selectedRouteId) {
  ...
  document.getElementById('excursionDate').addEventListener('change', () => updatePricing(guide));

  document.getElementById('excursionTime').value = '17:00';
  document.getElementById('excursionTime').addEventListener('change', () => updatePricing(guide));
...
function searchRoutes(e) {
  ...
  filteredRoutes = routesData.filter((route) => route.name.toLowerCase().includes(searchKeyword)
